...
(defun elpaca-install-from-cask (cask-file)
  "Read a CASK-FILE and install packages with Elpaca."
  (with-temp-buffer
    (insert-file-contents cask-file)
    (goto-char (point-min))
    (while (re-search-forward "(depends-on \\(\"\\([^\"]+\\)\"\\))" nil t)
      (let ((pkg (match-string 2)))
        (message "Installing %s via Elpaca..." pkg)
        (eval `(elpaca ,(intern pkg)))))))
...
version 2016-06-15"
  ...
  (let ((n (if (null n) 1 n)))
    (if (equal n 1)
        (if (or (equal (go-to-char) (line-beginning-position))
...
version 2016-06-15"
  ...
  (let ((n (if (null n) 1 n)))
    (if (equal n 1)
        (if (or (equal (go-to-char) (line-end-position))
                (equal last-command this-command )
